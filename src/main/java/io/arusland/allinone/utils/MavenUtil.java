    ...
    public static void goOffline2(File pomFile, File repositoryDir, List<Message> messages) {
        ...
        try {
            ...
            if (exitVal == 0) {
                ...
                String msg = IOUtils.toString(process.getErrorStream(), StandardCharsets.UTF_8);

                messages.add(new Message(StringUtils.isNotBlank(msg) ? msg : "Resolving failed: " + exitVal, "error"));
            }
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
